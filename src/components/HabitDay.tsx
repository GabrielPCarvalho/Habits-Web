...
export function HabitDay({defaultCompleted = 0, amount = 0, date}: HabitDayProps) {
  ...
  return (
    <Popover.Root>
    <Popover.Trigger
      className={clsx('w-10 h-10  border-2  rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-teal-700 focus:ring-offset-2 focus:ring-offset-background', {
    ...
    <Popover.Portal>
      <Popover.Content className='min-w-[320px] p-6 rounded-2xl bg-zinc-800 flex flex-col'>
        ...
        <span className='m-1 font-extrabold leading-tight text-3xl'>{dayAndMonth}</span>
